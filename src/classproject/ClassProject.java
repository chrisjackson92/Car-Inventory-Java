...
public class ClassProject {
    ...
    static public void menuSelection(int menuSelection, ArrayList<Car> carList) {
        if (menuSelection == 1) {
            ...
            Car tempCar = new Car(vin, model, year, mileage, price);
            carList.add(tempCar);
            System.out.println("Car added.");
        }
        if (menuSelection == 2) {
            System.out.println("Enter vin number of car to remove.");
            String vin = input.next();
            Car tempCar = new Car();
            for (Car car : carList) {
        ...
        if (menuSelection == 3) {
            ...
            System.out.printf("Car Edit List" + "1. Vin%n" + "2. Model%n" + "3. Year%n" + "4. Mileage%n"
                    + "5. Price%n%n" + "Enter edit choice: ");
        ...
        if (menuSelection == 7) {
            ...
            int carChoice = input.nextInt();
            Car car = carList.get(carChoice);
            System.out.printf("Car: %s%n" + "Price: $%.2f%n%n", car.getModel(), car.getPrice());
            System.out.printf(
                    "How do you feel about the car? (1. likes the car, 2. somewhat likes the car, 3. not easily swayed)");
            ...
            if (carSatisfaction == 1) {
                ...
                System.out.printf(
                        "Car: %s%n" + "Original price: $%.2f%n" + "discount: $%.2f%n" + "New price: $%.2f%n%n",
                        car.getModel(), currentPrice, 100.0, discountPrice);
            }
            if (carSatisfaction == 2) {
                ...
                System.out.printf(
                        "Car: %s%n" + "Original price: $%.2f%n" + "discount: $%.2f%n" + "New price: $%.2f%n%n",
                        car.getModel(), currentPrice, 200.0, discountPrice);
            }
            if (carSatisfaction == 3) {
                ...
                System.out.printf(
                        "Car: %s%n" + "Original price: $%.2f%n" + "discount: $%.2f%n" + "New price: $%.2f%n%n",
                        car.getModel(), currentPrice, 500.0, discountPrice);
            }
        }
    }
...
/*
 * • Ability to add a car to the array and set all of the values for it with
 * input from the user • Ability to ask the user which car in the array they
 * wish to change in some way, and then make that change. (Hint: Have it in a
 * separate method you are calling and print a message and ask them what they
 * want to change and number them so you can do a separate If/Else or Switch.
 * Then ask them for the value to change it to, and do it there. …don’t try to
 * do it in the main menu.) • Display a message with all the data for a car the
 * user chooses. • Display the data for all the cars (currently in the array)
 * for the user. • Display the average mileage for all of the cars on the lot. •
